...
export class AddCompetitionComponent {
  ...
  addCompetition(){
   ...
   if(this.validate()){
    this.competitionService.addCompetition(this.competition).subscribe(
      (data)=>{
        this.router.navigate(['/competitions/'+data.code]);
  ...
  validate(){
    if(this.competition.location==''
      ...
      || this.competition.endTime==null
      || this.competition.numberOfParticipants==null
      ){
        this.showAlertMessage("Please fill all the fields","warning");
      return false;
      }
    if(this.competition.startTime>this.competition.endTime){
    ...
    if(this.competition.numberOfParticipants<=0){
